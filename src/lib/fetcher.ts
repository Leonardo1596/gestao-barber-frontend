	...
	try {
		if (user && user.barbershop) {
			const response = await api.get(
				`/services/barbershop/${user.barbershop}`
			);
			return response.data;
		}
	} catch (err) {
...
export async function fetchProducts() {
	...
	try {
		const response = await api.get(
			`/products/barbershop/${user.barbershop}`
		);
		return response.data;
	} catch (err) {
...
export async function fetchAppointments(startDate: Date, endDate: Date) {
	...
	try {
		if (user && user.barbershop) {
			const response = await api.get(
				`/appointments-by-date/barbershop/${user.barbershop}?startDate=${from}&endDate=${to}`
			);
			return response.data;
		}
	} catch (err) {
...
export async function fetchTransactions(startDate: Date, endDate: Date) {
	...
	try {
		if (user && user.barbershop) {
			const response = await api.get(
				`/transactions-by-period/barbershop/${user.barbershop}/${from}/${to}`
			);
...
export async function updateAppointment(
	...
	try {
		await api.put(
			`/update-appointment/${selectedAppointment._id}`,
			updatedAppointment
		);
		return true;
	} catch (err) {
